...
type FAQ = {
  id?: number;
  question: string;
  answer?: string; // fallback name
  Answer?: string; // Strapi sometimes uses capital "A"
};
...
export default function FAQsSection({ faqs, title = 'FAQS' }: FAQsSectionProps) {
        ...
        [...Array(6)].map((_, i) => {
          ...
          return (
            <motion.div
              ...
              animate={
                isInView
                  ? { x: targetX, y: targetY, opacity: [0, 0.3, 0] }
                  : { opacity: 0 }
              }
              transition={{
